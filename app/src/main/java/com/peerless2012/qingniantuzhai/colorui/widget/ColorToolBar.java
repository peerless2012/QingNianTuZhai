...
public class ColorToolBar extends Toolbar implements ColorUiInterface {
    ...
    public void setTheme(Resources.Theme themeId, ThemeInfo themeInfo) {
        ...
        int toolbarStyleRes = themeInfo.getToolbarStyle();
        TypedArray toolbarStyleArray = getContext().obtainStyledAttributes(toolbarStyleRes, android.support.v7.appcompat.R.styleable.Toolbar);
        int popThemeArrayResourceId =toolbarStyleArray.getResourceId(android.support.v7.appcompat.R.styleable.Toolbar_popupTheme, R.style.ToolBarOverFlowLightStyle);
        setPopupTheme(popThemeArrayResourceId);
        getMenu().clear();
        inflateMenu(R.menu.activity_main);
        int titleTextColorRes = toolbarStyleArray.getResourceId(android.support.v7.appcompat.R.styleable.Toolbar_titleTextColor, R.color.main_textcolor_invert_normal);
        int titleTextColor = getResources().getColor(titleTextColorRes);
        setTitleTextColor(titleTextColor);
        // TODO overflow主题还不能随着更改，待解决。
        toolbarStyleArray.recycle();
